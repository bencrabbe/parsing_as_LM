...
class Beam:
    ...
    def push_next(self,sentence):
        """
        Pushes a new empty beam on top for next time step.
        Args:
             sentence (list): list of tokens to be parsed
        """
        for elt in self.beam[-1]:
            if elt.configuration is None:
                elt.exec_action(sentence)
        self.beam.append([])
        
    def extend_top(self,elt_list):
        """
        Extends the top beam with a list of additional BeamElement
        Args:
            elt_list (list): list of BeamElement
        """
        self.beam[-1].extend(elt_list)
    ...
    def discriminative_sample(self,K):
        ...
        samp_idxes = npr.choice(list(range(len(self.beam[-1]))),size=K,p=probs,replace=False)
...
if __name__ == '__main__':
    ...
    parser.train_model(train_treebank,dev_treebank,'test_rnngf/test_rnngf_gpu',epochs=20,lr=0.3,batch_size=32)
