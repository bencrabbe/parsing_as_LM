...
class RNNGlm:
    ...
    def init_ext_embedding_matrix(self,emb_wordlist,matrix):
        ...
        for emb_word,emb_vec in zip(emb_wordlist,matrix):
            idx = self.lexicon.index(emb_word,-1)
            if idx != self.lexicon.UNK_ID:
                new_mat[idx,:] = emb_vec                
        return new_mat
...
if __name__ == '__main__':
    ...
    rnnlm.train_rnn_lm('testlm',train_treebank,dev_treebank,lr=0.00001,dropout=0.5,batch_size=200,max_epochs=350,cls_filename='ptb-1000.brown',w2v_file='word_embeddings/w2v-ptb.txt')    
